...
class BaseView extends React.Component {
  ...
  constructor(props) {
    ...
    this.resourceLoader = props.resourceLoader;
  }

  setNavballOrientation(focus, navball) {
  ...
  _onWindowResize(cameras, originalHeight, originalFov) {
 ...
 */
  _adjustCoordinates(focus, position) {
    if (!focus) {
  ...
  _applyPlanetaryRotation(planet, body) {
  ...
  _defaultKeyPressHandler(solarSystem, additionalKeys) {
    const keyHandlers = {
    // -  (Decrease Throttle)
      45: (body) => {
    ...
    return ((key) => {
      const body = solarSystem.find(this.props.focus);
  ...
  _lookupNearbyBodies(focus, bodies, nearbyThreshold) {
